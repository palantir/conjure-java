 ...
 * the network. For example, an endpoint which schedules results on a single-threaded {@link
 * java.util.concurrent.ScheduledExecutorService} would congest the executor and fail to execute other scheduled work at
 * the expected time. We use {@link HttpServerExchange#dispatch(HttpHandler)} to put work onto the server task pool
 * where it is executed as similarly as possible to synchronous conjure endpoints to avoid behavior differences between
 * the two.
 */
final class ConjureAsyncRequestProcessing implements AsyncRequestProcessing {
    ...
    private static final ExchangeCompletionListener COMPLETION_LISTENER = SafeExchangeCompletionListener.of(
            exchange -> {
                ...
                new FutureCallback<T>() {
                    ...
                    public void onFailure(Throwable throwable) {
                        exchange.dispatch(wrapCallback(
                                serverExchange -> ConjureExceptions.handle(
                                        serverExchange, exceptionSerializer, throwable),
                                tracer));
                    }
                },
                DIRECT_EXECUTOR));
