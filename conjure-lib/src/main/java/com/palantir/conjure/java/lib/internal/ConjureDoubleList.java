...
final class ConjureDoubleList extends ImmutableList<Double> implements RandomAccess {
    ...
    final class Serializer extends JsonSerializer<ConjureDoubleList> {
        @Override
        public void serialize(ConjureDoubleList _val, JsonGenerator gen, SerializerProvider _serializer)
                throws IOException {
            double[] primitive = delegate.toArray();
            gen.writeArray(primitive, 0, primitive.length);
        }
    }
