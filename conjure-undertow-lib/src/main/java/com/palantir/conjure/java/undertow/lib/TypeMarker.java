...
public abstract class TypeMarker<T> {
    ...
    protected TypeMarker() {
        ...
        Preconditions.checkArgument(
                genericSuperclass instanceof ParameterizedType, "Class is not parameterized", SafeArg.of(
                        "class", genericSuperclass));
        type = ((ParameterizedType) genericSuperclass).getActualTypeArguments()[0];
        Preconditions.checkArgument(
                !(type instanceof TypeVariable), "TypeMarker does not support variable types", SafeArg.of(
                        "typeVariable", type));
    }
