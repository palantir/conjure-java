    ...
    public Set<JavaFile> generateTypes(List<TypeDefinition> types) {
        ...
        return types.stream().map(typeDef -> {
            if (typeDef.accept(TypeDefinitionVisitor.IS_OBJECT)) {
                return BeanGenerator.generateBeanType(typeMapper, typeDef.accept(TypeDefinitionVisitor.OBJECT));
            } else if (typeDef.accept(TypeDefinitionVisitor.IS_UNION)) {
                return UnionGenerator.generateUnionType(typeMapper, typeDef.accept(TypeDefinitionVisitor.UNION));
                ...
                return AliasGenerator.generateAliasType(typeMapper, typeDef.accept(TypeDefinitionVisitor.ALIAS));
            } else {
