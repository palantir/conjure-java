...
public final class ErrorGenerator {
    ...
    private static Map<String, Map<ErrorNamespace, List<ErrorDefinition>>> splitErrorDefsByNamespace(
        ...
        Map<String, Map<ErrorNamespace, List<ErrorDefinition>>> pkgToNamespacedErrorDefs = Maps.newHashMap();
        ...
        List<MethodSpec> checkMethodSpecs = errorTypeDefinitions.stream()
                .map(entry -> {
                    ...
                    Streams.concat(entry.getSafeArgs().stream(), entry.getUnsafeArgs().stream()).forEach(arg -> {
                        ...
                        methodBuilder.addJavadoc("@param $L $L", arg.getFieldName().get(), StringUtils.appendIfMissing(
                                arg.getDocs().map(Javadoc::render).orElse(""), "\n"));
                    });

                    methodBuilder.addCode("if ($L) {", shouldThrowVar);
                    methodBuilder.addCode("throw $L;", Expressions.localMethodCall(exceptionMethodName, Streams.concat(
                                    entry.getSafeArgs().stream(), entry.getUnsafeArgs().stream())
    ...
    private static void processArg(
        ...
        argDefinition.getDocs().ifPresent(
                docs -> methodBuilder.addJavadoc("@param $L $L", argName, Javadoc.render(docs)));
    }
