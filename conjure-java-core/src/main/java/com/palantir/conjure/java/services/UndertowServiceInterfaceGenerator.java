...
final class UndertowServiceInterfaceGenerator {
    ...
    public JavaFile generateServiceInterface(
        ...
        List<TypeSpec> endpointHandlers = serviceDefinition.getEndpoints().stream()
                .map(e -> generateEndpointHandler(
    ...
    private TypeName endpointToHandlerType(com.palantir.conjure.spec.TypeName serviceName, EndpointName name) {
        return ClassName.get(
                ...
                "Undertow" + serviceName.getName(),
                endpointToHandlerClassName(name));
    }

    private static String endpointToHandlerClassName(EndpointName name) {
    ...
    private static CodeBlock generateReturnValueCodeBlock(
    ...
    private static CodeBlock generateParamMarkers(
            List<Type> markers,
            String paramName,
    ...
    private static Optional<String> addAuthCode(CodeBlock.Builder code, EndpointDefinition endpointDefinition) {
    ...
    private static void addPathParamsCode(
    ...
    private static void addHeaderParamsCode(
            CodeBlock.Builder code,
            EndpointDefinition endpointDefinition,
            List<TypeDefinition> typeDefinitions,
            TypeMapper typeMapper) {
    ...
    private static void addQueryParamsCode(
            CodeBlock.Builder code,
            EndpointDefinition endpointDefinition,
            List<TypeDefinition> typeDefinitions,
            TypeMapper typeMapper) {
    ...
    private static boolean hasPathArgument(List<ArgumentDefinition> args) {
        return args.stream().anyMatch(arg -> arg.getParamType().accept(ParameterTypeVisitor.IS_PATH));
    }

    private static boolean hasQueryArgument(List<ArgumentDefinition> args) {
        return args.stream().anyMatch(arg -> arg.getParamType().accept(ParameterTypeVisitor.IS_QUERY));
    }

    private static boolean hasHeaderArgument(List<ArgumentDefinition> args) {
        return args.stream().anyMatch(arg -> arg.getParamType().accept(ParameterTypeVisitor.IS_HEADER));
    }

    private static CodeBlock generatePathParameterCodeBlock(
    ...
    private static CodeBlock generateQueryParameterCodeBlock(
            EndpointDefinition endpoint, List<TypeDefinition> typeDefinitions, TypeMapper typeMapper) {
        return generateParameterCodeBlock(
                endpoint,
    ...
    private static CodeBlock generateHeaderParameterCodeBlock(
            EndpointDefinition endpoint, List<TypeDefinition> typeDefinitions, TypeMapper typeMapper) {
        return generateParameterCodeBlock(
                endpoint,
    ...
    private static CodeBlock generateParameterCodeBlock(
    ...
    private static CodeBlock decodePlainParameterCodeBlock(
     ...
     */
    private static Optional<CodeBlock> getComplexTypeStringDeserializer(
            Type type, TypeMapper typeMapper, String resultVarName, String paramsVarName, String paramId) {
     ...
     * types and external imports.
     */
    private static Type getComplexType(Type type) {
    ...
    private static Optional<ArgumentDefinition> getBodyParamTypeArgument(List<ArgumentDefinition> args) {
        return args.stream()
