...
public final class AliasGenerator {
    ...
    public static JavaFile generateAliasType(TypeMapper typeMapper, AliasDefinition typeDef) {
        ...
        return JavaFile.builder(typePackage, spec.build()).skipJavaLangImports(true).indent("    ").build();
    }
        ...
        if (conjureType.accept(TypeVisitor.IS_PRIMITIVE)
            ...
            return typeMapper
                    ...
                    .flatMap(type -> valueOfFactoryMethod(
                            ...
                            .map(ignored -> {
                                ClassName className =
                                        ClassName.get(type.getTypeName().getPackage(), type.getTypeName().getName());
        ...
        switch (primitiveType.get()) {
            ...
            case DOUBLE:
                return CodeBlock.builder()
                        ...
                        .build();
            case INTEGER:
                return CodeBlock.builder().addStatement("return of($T.parseInt(value))", aliasTypeName.box()).build();
            case BOOLEAN:
                return CodeBlock.builder()
            ...
            case RID:
            case BEARERTOKEN:
                return CodeBlock.builder().addStatement("return of($T.valueOf(value))", aliasTypeName).build();
            case UUID:
                return CodeBlock.builder().addStatement("return of($T.fromString(value))", aliasTypeName).build();
            case DATETIME:
                return CodeBlock.builder().addStatement("return of($T.parse(value))", aliasTypeName).build();
            case BINARY:
            case ANY:
            case UNKNOWN:
        }
