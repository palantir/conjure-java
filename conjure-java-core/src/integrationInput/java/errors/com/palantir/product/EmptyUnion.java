...
public final class EmptyUnion {
    ...
    static final class Deserializer extends ConjureUnionDeserializer<EmptyUnion> {
        ...
        protected EmptyUnion deserializeSelected(JsonParser parser, DeserializationContext context, String type)
            ...
            if (variantIndex < 0) {
                return new EmptyUnion(new UnknownWrapper(type, deserializeUnknown(parser, context)));
            }
            return new EmptyUnion((Base) deserializeVariant(parser, context, variantIndex));
        }
    }
}
