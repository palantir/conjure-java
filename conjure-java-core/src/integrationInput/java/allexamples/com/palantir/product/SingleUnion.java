...
public final class SingleUnion {
    ...
    static final class Deserializer extends ConjureUnionDeserializer<SingleUnion> {
        private static final Class<?>[] VARIANT_TYPES = new Class<?>[] {FooWrapper.class};

        Deserializer() {
            super(SingleUnion.class, VARIANT_TYPES);
        }

        @Override
        protected SingleUnion deserializeSelected(JsonParser parser, DeserializationContext context, String type)
                throws IOException {
            int variantIndex =
                    switch (type) {
                        case "foo" -> 0;
            ...
            if (variantIndex < 0) {
                return new SingleUnion(new UnknownWrapper(type, deserializeUnknown(parser, context)));
            }
            return new SingleUnion((Base) deserializeVariant(parser, context, variantIndex));
        }
    }
}
