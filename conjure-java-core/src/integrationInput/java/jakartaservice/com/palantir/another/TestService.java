...
import jakartaservice.com.palantir.product.AliasedString;
import jakartaservice.com.palantir.product.CreateDatasetRequest;
import jakartaservice.com.palantir.product.datasets.BackingFileSystem;
import jakartaservice.com.palantir.product.datasets.Dataset;
...
public interface TestService {
    ...
    void uploadRawData(@HeaderParam("Authorization") AuthHeader authHeader, @Safe InputStream input);

    @POST
    @Path("catalog/datasets/upload-raw-aliased")
    @Consumes(MediaType.APPLICATION_OCTET_STREAM)
    @ClientEndpoint(method = "POST", path = "/catalog/datasets/upload-raw-aliased")
    void uploadAliasedRawData(@HeaderParam("Authorization") AuthHeader authHeader, InputStream input);
